...
import com.edu.kygroup.ui.BaiduMapActivity;
import com.edu.kygroup.ui.ChatActivity1;
import com.edu.kygroup.ui.KygroupApplication;
import com.edu.kygroup.ui.ShowBigImage;
import com.edu.kygroup.ui.ShowVideoActivity;
import com.edu.kygroup.utils.Constant;
import com.edu.kygroup.utils.ImageCache;
import com.edu.kygroup.utils.ImageUtils;
import com.edu.kygroup.utils.LoadImageTask;
import com.edu.kygroup.utils.LoadVideoImageTask;
...
public class MessageAdapter1 extends BaseAdapter {
	...
	public MessageAdapter1(Context context, String username, int chatType) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
				...
				public void onClick(View v) {
					...
					if (message.getType() == EMMessage.Type.TXT)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_TEXT);
					else if (message.getType() == EMMessage.Type.VOICE)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_VOICE);
					else if (message.getType() == EMMessage.Type.IMAGE)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_PICTURE);
					else if (message.getType() == EMMessage.Type.LOCATION)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_LOCATION);
					else if (message.getType() == EMMessage.Type.FILE)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_FILE);
					else if (message.getType() == EMMessage.Type.VIDEO)
						activity.startActivityForResult(intent,
								ChatActivity1.REQUEST_CODE_VIDEO);
		...
		if (message.direct == EMMessage.Direct.SEND) {
			mImageLoader.displayImage(KygroupApplication.mUser.getPic(),
					holder.head_iv, mOptions);
			System.out.println("pic::::::::::::"
					+ KygroupApplication.mUser.getPic());
		} else {
			User user = new User();
			user.setEmail(message.getFrom());
			user.getNameAndAvatar(DBUtils.getDBsa(1));
			if (chatType == ChatType.GroupChat) {
				holder.tv_userId.setText(user.getNickName());
			}
			mImageLoader.displayImage(user.getPic(), holder.head_iv, mOptions);
		}
		return convertView;
	}
	...
	private void sendPictureMessage(final EMMessage message,
		...
		try {
			...
			holder.tv.setVisibility(View.VISIBLE);
			holder.tv.setText("0%");
			// if (chatType == ChatActivity1.CHATTYPE_SINGLE) {
			EMChatManager.getInstance().sendMessage(message, new EMCallBack() {
